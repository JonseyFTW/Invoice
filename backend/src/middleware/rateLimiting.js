...
const rateLimitConfigs = {
  ...
  auth: {
    ...
    keyGenerator: (req) =>
      ...
      `auth:${req.ip}:${req.get('User-Agent') || 'unknown'}`,
...
const createDynamicRateLimiter = (getUserType) => async (req, res, next) => {
  try {
...
const createAdaptiveRateLimiter = () => (req, res, next) => {
...
const getRateLimitStatus = async (req, res) => {
  try {
    ...
    const { windowMs } = rateLimitConfigs.api;
